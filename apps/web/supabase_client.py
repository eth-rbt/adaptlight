...
Stores each text command along with the full state machine snapshot
...
def submit_feedback(
    ...
    try:
        ...
        print(f"Feedback submitted for session: {session_id}")
        return True
    except Exception as e:
...
def get_user_sessions(user_id: str, limit: int = 50) -> List[Dict]:
    ...
    try:
        result = client.table('command_sessions') \
            .select('*') \
            .eq('user_id', user_id) \
