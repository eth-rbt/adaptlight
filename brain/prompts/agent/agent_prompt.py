...
- **PoseNet confidence guideline (single-hand proximity):** default hand landmark confidence threshold should be low (`~0.03–0.10`, prefer `0.05`) unless user asks for strict filtering. Do NOT default to `0.3+` for wrist/hand points on low-confidence streams.**
- **Calibration guideline:** for hand-size brightness mappings, set `map_range` input bounds from observed runtime area values (not fixed assumptions), and keep a clamp-backed fallback path so dim state is stable when landmarks are sparse.**
